...
- CVE-2026-44578 Next.js WebSocket SSRF: absolute-form URI + Upgrade: websocket headers → internal service proxy (CVSS 8.6, affects 13.4.13–15.5.15 and 16.0.0–16.2.4, self-hosted only)
- AWS Lambda Runtime API SSRF: localhost:9001 endpoint returns live event payload + IAM temp creds; distinct from EC2 IMDS; no auth required
...
## Threat model — current state (2026-05-30)
...
NEW (2026-05-30): CVE-2026-44578 (Next.js WebSocket SSRF, CVSS 8.6) adds a second Next.js critical
SSRF in 12 months — GET-only, IMDSv1 immediately exploitable, single raw TCP request.
Lambda Runtime API (localhost:9001) is the serverless SSRF target — distinct from EC2 IMDS, missed by
standard filters. Nginx off-by-slash proxy_pass traversal is pervasive in microservice gateway configs.
Envoy sidecar admin API (localhost:15000) in Istio meshes leaks mTLS keys enabling full identity siphon.
HTTP request smuggling CL.TE/TE.CL desync provides backend SSRF that bypasses all front-end controls.
...
1. **SKILL_SSRF.md** — ✓ Updated 2026-05-30: +5 techniques (CVE-2026-44578 Next.js WebSocket, Lambda Runtime API, Nginx off-by-slash, Envoy sidecar, HTTP smuggling CL.TE), +5 bypass matrix rows, +5 HVT rows, 29-item threat model (fixed numbering)
2. **SKILL_XSS.md** — Add Trusted Types bypass (policy injection), Shadow DOM XSS (open shadow root injection), service worker injection for persistent XSS, XSS via CSS expression()/-moz-binding (legacy IE/Firefox)
3. **SKILL_IDOR.md** — ✓ Updated 2026-05-28: +17 techniques total, 9 chains, Threat Model 18 entries, Bypass Matrix +17 rows, HVT +16 rows
4. **NEW: SKILL_AUTH.md** — OAuth 2.0 attack surface (state param, redirect_uri, implicit flow token leak), JWT attacks (alg:none, jwks_uri SSRF, kid injection), SAML attacks, password reset flaws, session fixation
5. **NEW: SKILL_SQLI.md** — Modern SQLi (JSON operators, out-of-band exfil via DNS, WAF bypass via chunked encoding, second-order injection, polyglot payloads)
6. **nuclei-templates/** — ✓ CVE-2025-29927 (Next.js auth bypass) added 2026-05-26.
   Remaining: CVE-2026-44578 (Next.js WebSocket SSRF), CVE-2025-61882 (Oracle EBS RCE),
   Spring4Shell (CVE-2022-22965), ProxyLogon (CVE-2021-26855),
   Confluence RCE (CVE-2022-26134), Text4Shell (CVE-2022-42889),
   Apache Struts RCE (CVE-2017-5638), GitLab SSRF/RCE
